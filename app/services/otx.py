...
OTX_DOMAIN_URL = "https://otx.alienvault.com/api/v1/indicators/domain"
...
async def fetch_otx_domain(domain: str) -> tuple[dict | None, SourceMetadata]:
    source = SourceMetadata(
        source="alienvault_otx",
        retrieved_at=utc_now(),
        success=False
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(
                f"{OTX_DOMAIN_URL}/{domain}/general",
                headers={
                    "X-OTX-API-KEY": settings.otx_api_key,
                    "Accept": "application/json"
                },
